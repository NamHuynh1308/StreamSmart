...
// AchievementsPanel is a page that shows user achievements and progress
interface AchievementsPanelProps {
  isOpen: boolean;
  onClose: () => void;
}

// Mock achievement data, progress on a 0 to 1 scale, EX.  0.4 means 40%.
...
export function AchievementsPanel({ isOpen, onClose }: AchievementsPanelProps) {
  ...
  // Return the JSX that renders the achievements panel
  return (
              ...
              {notAchieved.map(a => (
                <div key={a.id} className="bg-zinc-800 p-3 rounded border border-zinc-700">
                  ...
                  <div className="w-full bg-zinc-700 h-2 rounded overflow-hidden">
                    {/* Progress bar fullness based on the achievement percent */}
